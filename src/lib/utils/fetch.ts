...
export type RequestResult<S extends object, E extends object = Response> = ResultAsync<
  S,
  RequestError<E>
>;
...
function handleNetworkError(error: unknown): Extract<RequestError, { type: "NETWORK_ERROR" }> {
  console.error(error);
  return {
    type: "NETWORK_ERROR",
...
export function fetchResultWithError<S extends object, E extends object>(
  ...
  return ResultAsync.fromPromise(fetch(...args), handleNetworkError)
    .andThen((res) => {
      if (!res.ok) {
        return parseJson<E>(res).andThen(
          (error) => err({ type: "RESPONSE_ERROR", error } as const),
        );
      }
